# Demo: `changesets-format-with-issue-links` with Changesets 2.x
...
This is a small demo package that exercises `changesets-format-with-issue-links` against Changesets 2.x.
...
To run locally:
```bash
scripts/run-external-test.sh  demo-changesets-2.x
```

or, to get a shell and play around on your own:
```bash
scripts/run-external-test-interactive.sh  demo-changesets-2.x
```
